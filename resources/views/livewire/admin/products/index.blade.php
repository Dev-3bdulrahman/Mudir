<div>
    <div class="flex items-center justify-between mb-8">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ __('Manage Products') }}</h2>
            <p class="text-gray-500 dark:text-gray-400 text-sm">{{ __('Manage and display your items') }}</p>
        </div>
        <button wire:click="create" class="bg-green-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-green-700 transition-colors">
    ...
    <div class="grid md:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-900 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 flex items-start gap-4 hover:shadow-md transition-shadow">
            <div class="w-12 h-12 rounded-lg bg-{{ $product['color'] ?? 'green' }}-50 dark:bg-{{ $product['color'] ?? 'green' }}-900/20 flex items-center justify-center text-{{ $product['color'] ?? 'green' }}-600 dark:text-{{ $product['color'] ?? 'green' }}-400 flex-shrink-0">
                <i data-lucide="package" class="w-6 h-6"></i>
            </div>
            <div class="flex-1">
                <div class="flex items-center justify-between mb-2">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">{{ is_array($product['title']) ? ($product['title'][app()->getLocale()] ?? array_shift($product['title'])) : $product['title'] }}</h3>
                ...
                <div class="mt-4 pt-4 border-t border-gray-50 dark:border-gray-800 flex items-center justify-between">
                    <span class="text-xs font-medium text-gray-400 dark:text-gray-500 italic">تم المزامنة من API</span>
                    <span class="text-xs font-bold text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/30 px-2 py-0.5 rounded">Active</span>
                </div>
            </div>
        </div>
        @endforeach
        ...
        <div class="relative flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="open"
                ...
                <form wire:submit="save">
                    <div class="bg-white dark:bg-gray-900 px-6 py-6 border-b border-gray-100 dark:border-gray-800">
                        <div class="flex items-center justify-between">
                            <h3 class="text-xl font-bold text-gray-900 dark:text-white">
                                {{ $productId ? __('Edit Product') : __('Add New Product') }}
                            </h3>
                    ...
                    <div class="bg-white dark:bg-gray-900 px-6 py-6 space-y-6 text-right">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="p-title_ar" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Product Name') }} (العربية)</label>
                                <input wire:model="title_ar" type="text" id="p-title_ar" dir="rtl"
                                       class="w-full px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all outline-none">
                                @error('title_ar') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label for="p-title_en" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Product Name') }} (English)</label>
                                <input wire:model="title_en" type="text" id="p-title_en" dir="ltr"
                                       class="w-full px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all outline-none">
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label for="p-description_ar" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Product Description') }} (العربية)</label>
                                <textarea wire:model="description_ar" id="p-description_ar" rows="4" dir="rtl"
                                          class="w-full px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all outline-none"></textarea>
                                @error('description_ar') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label for="p-description_en" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('Product Description') }} (English)</label>
                                <textarea wire:model="description_en" id="p-description_en" rows="4" dir="ltr"
                                          class="w-full px-4 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-900 dark:text-white focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all outline-none"></textarea>
                        ...
                        <div>
                            ...
                            <div class="flex flex-wrap gap-3">
                                @foreach(['green', 'blue', 'purple', 'cyan', 'indigo', 'orange', 'red'] as $c)
                                <label class="relative cursor-pointer">
                                    <input wire:model="color" type="radio" name="color" value="{{ $c }}" class="sr-only peer">
                                    <div class="w-8 h-8 rounded-full bg-{{ $c }}-500 border-2 border-transparent peer-checked:border-white peer-checked:ring-2 peer-checked:ring-{{ $c }}-500 transition-all"></div>
                                </label>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-800/50 px-6 py-4 flex flex-row-reverse gap-3">
                        <button type="submit" 
                                class="w-full inline-flex justify-center rounded-lg border border-transparent shadow-sm px-6 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:w-auto sm:text-sm transition-colors">
                            {{ __('Save Changes') }}
                        </button>
            ...
            <div class="inline-block align-middle bg-white dark:bg-gray-900 rounded-xl text-right overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <div class="p-6">
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white">حذف المنتج</h3>
                    </div>
                    <p class="text-gray-600 dark:text-gray-400 mb-8">هل أنت متأكد من رغبتك في حذف هذا المنتج؟</p>
